...
def list_of_databases():
	...
	def pipi(i,li):
		print('['+str(i)+']'+'  '+ li)
	[pipi(i,l[i]) for i in range(0,len(l))]
...
def strategy_maker():
	...
	with open('blocks_01_background.txt') as f:
		bgg=f.read()
	b=bgg.split('block')

	with open('blocks_02_profile.txt') as f:
		prof=f.read()
	p=prof.split('block')

	with open('blocks_03_atomics.txt') as f:
		ato=f.read()
	a=ato.split('block')

	nn=1
	with open('strategy_log.log','w') as fl:
		fl.writelines('strg\tblock of bgg\tblock of profile\tblock of atomics\n')

		for i in range(1,len(b)):
			for ii in range(1,len(p)):
				for iii in range(1,len(a)):
					with open('strg_'+str(nn)+'.ctrl','w') as f:
